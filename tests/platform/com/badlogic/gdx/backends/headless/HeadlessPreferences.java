...
public class HeadlessPreferences implements Preferences {
	...
	public HeadlessPreferences(File file) {
		...
		InputStream in = null;
		try {
			in = new BufferedInputStream(AndroidBackendHelper
					.getAndroidContext().openFileInput(this.file));
	...
	public void flush() {
		...
		try {
			...
			out = AndroidBackendHelper.getAndroidContext().openFileOutput(file,
